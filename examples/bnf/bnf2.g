...
we want to get a simple representation of the grammar as the set of rules
...
let str_of_SYM sym = match sym with 
  | NT nt -> "(NT(\""^nt^"\"))"
  | TM tm -> "(TM(\""^(String.escaped tm)^"\"))"

let str_of_SYMS alt = "[" ^ (String.concat ";" (List.map str_of_SYM alt)) ^ "]"

let str_of_ACT act = ""

let str_of_SYMSACT (alt,act) = "("^ (str_of_SYMS alt) ^")"

let str_of_RHS rhs = "["^ (String.concat ";" (List.map str_of_SYMSACT rhs)) ^"]"

let str_of_RULE (nt,rhs) = "("^(str_of_SYM (NT nt))^","^(str_of_RHS rhs)^")"

let str_of_RULES rs = (String.concat "\n\n" (List.map str_of_RULE rs))
